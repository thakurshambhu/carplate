...
<section class="slider_section slider_two">
    <div class="slider_area">
        <div class="single_slider d-flex align-items-center" data-bgimg="{{asset('images/banner.jpg')}}">
            <div class="container">
                <div class="row justify-content-center align-items-center">
                    <div class="col-12 col-md-8">
                        <h1 class="heading">PRIVATE <span>NUMBER PLATES</span></h1>
                        <div class="slider_content search-container search_two">
                            <form method="POST" action="{{url('searching_plates')}}">
                            @csrf
...
<section class="plat-number-count text-center">
    <h6><span>{{$plates}}</span> plates available now</h6>
...
<div class="banner-row">
    <div class="banner-col">
        @if(Auth::user())
            <a href="{{url('seller/plate_list')}}" class="block sell">
				<strong>SELL A PLATE</strong>
			</a>
            @else
            <a href="{{route('login')}}" class="block sell">
                <strong>SELL A PLATE</strong>
            </a>
        @endif
    </div>
    <div class="banner-col">
        <a href="{{url('plates')}}" class="block buy">
        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    ...
                    @if(!empty($feature_plates))
                        @foreach($feature_plates as $feature_plate)
                        <div class="single_blog">
                            <div class="blog_thumb">
                                <a href="javascript:void(0)" >
                                    <div class="primary_img view_plate" data-toggle="modal" data-target="#modal_box" title="quick view" view-plate-id="{{$feature_plate->id}}">{{$feature_plate->plate_number}}</div>
                                    <p>
                                        <strong>{{$feature_plate->plate_title}}</strong>
                                        <span class="more">${{$feature_plate->plate_price}}</span>
                                    </p>
                                </a>
                            </div>
                        </div>
